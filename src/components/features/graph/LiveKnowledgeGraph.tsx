...
export function LiveKnowledgeGraph() {
  ...
  return (
    <div className="space-y-8" dir={isFa ? "rtl" : "ltr"} role="group" aria-labelledby="live-graph-title" aria-describedby="live-graph-description">
      ...
      <div className="text-center max-w-4xl mx-auto space-y-4">
        <span className="inline-flex items-center gap-1.5 text-[11px] uppercase font-black tracking-widest text-sky-400 bg-sky-500/10 px-4 py-1.5 rounded-full border border-sky-500/15">
          ...
          {isFa ? "دموی نمایشی پوشش معنایی" : "ILLUSTRATIVE SEMANTIC VISIBILITY DEMO"}
        </span>
        <h2 id="live-graph-title" className="font-display font-black text-3xl md:text-5xl text-white">
          {isFa ? "گراف زنده روابط" : "Live Semantic Discovery"}
        </h2>
        <p id="live-graph-description" className="text-slate-400 leading-relaxed text-sm md:text-base font-medium max-w-2xl mx-auto">
          {isFa
            ? "این نمودار تعاملی، داده نمایشی و ساختگی است و داده زنده مشتریان را نشان نمی‌دهد. پنج شاخص نمونه، پوشش استناد برند را در موتورهای هوش مصنوعی مقایسه می‌کنند."
            : "This interactive chart uses illustrative sample data, not live customer data. Five sample metrics compare brand citation coverage across AI engines."}
        </p>
      </div>
      ...
      <div className="bg-[#08111F] border border-white/[0.08] rounded-[2rem] shadow-[0_24px_60px_rgba(0,0,0,0.8)] backdrop-blur-3xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] pb-5 mb-5 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-0.5">
              <span className="text-xs uppercase font-extrabold tracking-wider text-white">
                {isFa ? "جریان داده‌های نمونه" : "SAMPLE TELEMETRY STREAM"}
              </span>
          ...
          <div className="flex items-center gap-2 text-[10px] bg-sky-500/10 text-sky-400 font-extrabold px-3 py-1 rounded-lg border border-sky-500/15">
            ...
            <span>{isFa ? "داده نمونه" : "SAMPLE DATA"}</span>
          </div>
        </div>
        ...
        <div className="relative w-full h-[280px] sm:h-[400px] z-10" ref={containerRef}>
          ...
          />
          <div className="sr-only" aria-label={isFa ? "داده‌های نمونه نمودار گراف معنایی" : "Illustrative semantic graph data"}>
            <p>{isFa ? "این فهرست، جایگزین متنی نمودار است و مقادیر نمونه را بیان می‌کند." : "Text alternative for the chart. Values are illustrative samples."}</p>
            <ul>
              {METRICS.map((metric) => (
                <li key={metric.id}>
                  {isFa
                    ? `${metric.nameFa}: ${formatFaNumber(metric.basePct)}٪، روند ${formatFaNumber(metric.trend)}`
                    : `${metric.nameEn}: ${metric.basePct.toFixed(1)}%, trend ${metric.trend}`}
                </li>
              ))}
            </ul>
          </div>
            ...
            {tooltip.visible && (
              ...
              >
                ...
                <div className="text-[9px] text-slate-500 font-bold flex items-center gap-1 mt-2.5 pt-2.5 border-t border-white/[0.04]">
                  ...
                  <span>{isFa ? "مقدار نمونه برای مدل" : "Illustrative model value"}</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="border-t border-white/[0.06] pt-6 mt-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
              {isFa ? "راهنمای شاخص‌های پایشگر شده" : "MONITORED METRIC SERIES"}
            </span>
        ...
        <div className="border-t border-white/[0.06] pt-6 mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-white/10 transition-all">
            <span className="text-[10px] text-slate-500 font-bold uppercase block mb-1">
              {isFa ? "نام مرجع پایش" : "Brand Reference Hub"}
            </span>

...
export async function testContentStudio() {
  ...
  try {
    ...
    // Verify 'آنالیز' was suggested for replacement
    const hasAuditCorrection = payloadA.terminologySuggestions.some(
      s => s.originalWord === "آنالیز" && s.suggestedWord.includes("تحلیل")
    );
    if (!hasAuditCorrection) {
      throw new Error(`Scenario A Failed: Expected 'آنالیز' to be flagged and replaced, got suggestions: ${JSON.stringify(payloadA.terminologySuggestions)}`);
    }
    ...
    console.log(`  * Success: Generated ${payloadA.generatedContent.length} drafts with semantic health score of ${payloadA.semanticHealthScore}%. Correctly flagged 'آنالیز'.`);
